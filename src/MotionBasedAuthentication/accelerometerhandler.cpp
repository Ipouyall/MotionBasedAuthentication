...
AccelerometerHandler::AccelerometerHandler(QObject *parent)
    : QObject(parent),
...
void AccelerometerHandler::readSensorValues()
{
    ...
    if (reading)
    {
        ...
        else
        {
            ...
            if (qAbs(adjustedX) < threshold)
                adjustedX = 0;
            if (qAbs(adjustedY) < threshold)
                adjustedY = 0;
            if (qAbs(adjustedZ) < threshold)
                adjustedZ = 0;
            ...
            if (qAbs(velocityX) < velocityThreshold && qAbs(velocityY) < velocityThreshold && qAbs(velocityZ) < velocityThreshold)
            {
                if (wasMoving)
                {
                    emit pathDataReady(intervalDistanceX, intervalDistanceY, intervalDistanceZ); // Pass 0 for angleZ, which will be handled in SensorHandler
...
void AccelerometerHandler::start()
{
    ...
    accelerometer->start();
    timer->start(50);
    QTimer::singleShot(4000, this, &AccelerometerHandler::calculateBias);
    elapsedTimer.restart();
}
